/**
 * React Wrapper for D3.js Area Chart
 *
 * Uses the same D3 engine as LineChartWrapper but with showArea=true.
 * When a color field is present, renders as stacked area.
 */
import React, { useRef, useEffect, useState } from 'react';
import { createLineChart, createTrellisLineChart } from './lineChart';
...
const convertData = (data) => (!data?.rows ? [] : data.rows);

const collapseLabels = (rows, labelFields, measures, fieldAggregations) => {
  if (!labelFields || labelFields.length === 0 || rows.length === 0) return rows;
  ...
  for (const g of groups.values()) {
    for (const m of measures) {
      const agg = (fieldAggregations[m] || 'sum').toLowerCase();
      if (agg === 'avg' || agg === 'average' || agg === 'mean') g[m] = g._count > 0 ? g[m] / g._count : 0;
      if (g[m] === null) g[m] = 0;
    }
    delete g._count;
    result.push(g);
...
const AreaChartWrapper = ({ data, config, query, fieldAggregations = {} }) => {
  ...
  useEffect(() => {
    ...
    const isTrellis = detailField && (Array.isArray(detailField) ? detailField.length > 0 : !!detailField) &&
    ...
    // Stacked when there's a color field or multiple measures
    const hasColorField = !!(query?.colorField || query?.marks?.color);
    const hasMultipleMeasures = (query?.measures || []).length > 1;
    ...
    const opts = {
      ...
      showArea: true,
      stacked: hasColorField || hasMultipleMeasures,
    ...
    if (isTrellis) {
      chartRef.current = createTrellisLineChart(containerRef.current, chartConfig, chartData, opts);
    } else {
      chartRef.current = createLineChart(containerRef.current, chartConfig, chartData, opts);
    }
...
export default React.memo(AreaChartWrapper);
