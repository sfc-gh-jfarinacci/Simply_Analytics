...
const queryToConfig = (query) => {
  ...
  return { x_axis: xAxis, categoryField: xAxis, series };
};

const convertData = (data) => data?.rows || [];

const BoxPlotChartWrapper = ({ data, config, query, fieldAggregations = {} }) => {
  ...
  useEffect(() => {
    ...
    chartRef.current = createBoxPlotChart(containerRef.current, chartConfig, rawData, {
...
export default React.memo(BoxPlotChartWrapper);
