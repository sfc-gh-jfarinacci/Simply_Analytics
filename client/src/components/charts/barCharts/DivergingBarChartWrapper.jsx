/**
 * React Wrapper for D3.js Diverging Bar Chart
 * 
 * Handles React lifecycle, container sizing, data aggregation, and updates
 */
import React, { useRef, useEffect, useState } from 'react';
import { createDivergingBarChart, createTrellisDivergingBarChart } from './divergingBarChart';
...
const queryToConfig = (query) => {
  ...
  if (query?.sorts) {
    query.sorts.forEach(s => {
      if (s.field) sorts[s.field] = s.direction || 'asc';
    });
  }
...
const DivergingBarChartWrapper = ({ data, config, query, fieldAggregations = {} }) => {
  ...
  useEffect(() => {
    ...
    if (isTrellis) {
      chartRef.current = createTrellisDivergingBarChart(containerRef.current, chartConfig, chartData, chartOptions);
    } else {
      chartRef.current = createDivergingBarChart(containerRef.current, chartConfig, chartData, chartOptions);
    }
...
export default React.memo(DivergingBarChartWrapper);
