...
export const smartAxisFormat = (value) => {
  const abs = Math.abs(value);
  if (abs === 0) return '0';
  if (abs >= 1e9) return d3.format('.3s')(value).replace('G', 'B');
  if (abs >= 1e6) return d3.format('.3s')(value).replace('G', 'B');
  if (abs >= 1e3) return d3.format(',.0f')(value);
...
export const createValueFormatter = (fieldFormats = {}) => (value, fieldName) => {
  ...
  switch (format) {
    ...
    case 'compact': return d3.format(`,.${getD(1)}s`)(value).replace('G', 'B');
    case 'currency': return '$' + Math.abs(value).toLocaleString('en-US', { minimumFractionDigits: getD(2), maximumFractionDigits: getD(2) });
    case 'percent': return (value * 100).toFixed(getD(1)) + '%';
    default:
      return value.toLocaleString('en-US', { maximumFractionDigits: getD(2) });
  }
};
